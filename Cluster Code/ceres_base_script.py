...
def compute():
    ...
    data_rh = xr.open_dataset('./Data/ERA5/era5_rh_2000_2020.nc').sel(latitude=lats, longitude=lons).sel(time=slice("2001-01-01", "2020-12-30"))
    ...
    for i_mon in range(len(month_list)):
        ...
        for j in range(len(times_list)):
            ...
            # Mean surface temperatures for the year of the selected time value
            ts_mean = np.mean(data_ts.sel(latitude=latval, longitude=lonval, time = data_ts['time.year'] == data_ts.sel(time=times_list[j])['time.year'].values[()]).t2m.values)
            
            # Difference between mean ts and actual ts value
            ts_diff = data_ts.sel(latitude=latval, longitude=lonval, time=times_list[j]).t2m.values[()] - ts_mean
            
            # Temperatures
            # Set atmospheric temperature to mean, shifted by ts_diff
            state.Tatm[:] = [tval + ts_diff for tval in data_t.t.sel(latitude=latval, longitude=lonval, level=slice(0,surf_p), time = data_t['time.year'] == data_t.sel(time=times_list[j])['time.year'].values[()]).mean(dim='time').values]
            ...
            rh_vals = data_rh.r.sel(latitude=latval, longitude=lonval, level=slice(0,surf_p), time = data_rh['time.year'] == data_rh.sel(time=times_list[j])['time.year'].values[()]).mean(dim='time').values
            
            # Calculate q values from these constant RH values
            t_vals = [tval + ts_diff for tval in data_t.t.sel(latitude=latval, longitude=lonval, level=slice(0,surf_p), time = data_t['time.year'] == data_t.sel(time=times_list[j])['time.year'].values[()]).mean(dim='time').values]
            q_vals = []
            for k in range(len(rh_vals)):
                q_vals.append(q_from_rh(rh=rh_vals[k], T=t_vals[k], p=levels_correct[k]))
    ...
    h.to_netcdf('./Output_Data_ceres_base/'+str(h_index)+'.nc')
